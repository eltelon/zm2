...
var debug = require('debug')('zm2:paths');
var p     = require('path');
var fs    = require('fs')
...
function getDefaultPM2Home() {
  ...
  if (process.env.ZM2_HOME)
    PM2_ROOT_PATH = process.env.ZM2_HOME;
  else if (process.env.PM2_HOME)
    PM2_ROOT_PATH = process.env.PM2_HOME;
  else if (process.env.HOME && !process.env.HOMEPATH)
    PM2_ROOT_PATH = p.resolve(process.env.HOME, '.zm2');
  else if (process.env.HOME || process.env.HOMEPATH)
    PM2_ROOT_PATH = p.resolve(process.env.HOMEDRIVE, process.env.HOME || process.env.HOMEPATH, '.zm2');
  else {
    console.error('[ZM2][Initialization] Environment variable HOME (Linux) or HOMEPATH (Windows) are not set!');
    console.error('[ZM2][Initialization] Defaulting to /etc/.zm2');
    PM2_ROOT_PATH = p.resolve('/etc', '.zm2');
  }

  debug('zm2 home resolved to %s', PM2_ROOT_PATH, process.env.HOME);
...
module.exports = function(PM2_HOME) {
  ...
  var pm2_file_stucture = {
    ...
    PM2_LOG_FILE_PATH        : p.resolve(PM2_HOME, 'zm2.log'),
    PM2_PID_FILE_PATH        : p.resolve(PM2_HOME, 'zm2.pid'),
    ...
    PM2_IO_ACCESS_TOKEN      : p.resolve(PM2_HOME, 'zm2-io-token'),
    DUMP_FILE_PATH           : p.resolve(PM2_HOME, 'dump.zm2'),
    DUMP_BACKUP_FILE_PATH    : p.resolve(PM2_HOME, 'dump.zm2.bak'),
    STATE_FILE_PATH          : p.resolve(PM2_HOME, 'state.json'),
