...
function buildExecStart(conf) {
  ...
  // Script path
  parts.push(conf.pm_exec_path || conf.script);

  // App args
  if (conf.args) {
    var appArgs = Array.isArray(conf.args) ? conf.args : String(conf.args).split(' ');
    parts = parts.concat(appArgs.filter(function(a) { return a; }));
  }
...
function generateEnvFile(conf) {
  ...
  Object.keys(env).forEach(function(key) {
    // Skip internal pm2 vars
    if (key.indexOf('pm_') === 0 || key === 'pm2_env') return;
    var val = env[key];
