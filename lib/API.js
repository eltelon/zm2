...
var Client      = require('./SystemdClient');
...
////////////////////////////
  ...
  update (cb) {
    var that = this;
    Common.printOut('ZM2 runs in systemd mode — no daemon to update.');
    Common.printOut('To update, run: npm install @zappinginc/zm2@latest -g');
    return cb ? cb(null, {success:true}) : that.speedList();
  }
  ...
  killDaemon (cb) {
    ...
    that._operate('deleteProcessId', 'all', function(err, list) {
      ...
      return cb ? cb(null, {success:true}) : that.exitCli(conf.SUCCESS_EXIT);
    })
  }
    ...
    function processIds(ids, cb) {
      ...
      eachLimit(ids, concurrent_actions, function(id, next) {
        ...
        that.Client.executeRemote(action_name, opts, function(err, res) {
          if (err) {
            Common.printError(conf.PREFIX_MSG_ERR + 'Process %s not found — real error: %s', id, err.message || err);
  ...
  _handleAttributeUpdate (opts) {
    ...
    if (!appConf || appConf instanceof Error) {
      Common.printError('Error while transforming CamelCase args to underscore');
      return appConf || new Error('verifyConfs returned empty');
    }
  ...
  speedList (code, apps_acted) {
    ...
    function doList(err, list) {
      if (err) {
        console.error('Error retrieving process list: %s', err);
        return that.exitCli(conf.ERROR_EXIT);
      }
...
API.prototype.api = function(opts, cb) {
  ...
  var StateStore = require('./Systemd/StateStore');
  var conf = require('../constants');
  var stateStore = new StateStore(path.join(that.pm2_home, 'state.json'));
  stateStore.load();
  ...
  switch (subcommand) {
    case 'start':
      ...
      try {
        ...
        stateStore.register('zm2-api', { name: 'zm2_api', script: scriptPath, pm_id: 0 });
      } catch (e) {
...
/**
 * Dump current process list to dump.zm2 for later resurrection
 */
API.prototype.dump = function(force, cb) {
  var that = this;
  ...
  var conf = that._conf || require('../constants');
  ...
  that.Client.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    var envList = (list || []).map(function(proc) {
      return proc.pm2_env;
    });

    // If list is empty and force is not set, warn
    if (envList.length === 0 && !force) {
      Common.printOut(conf.PREFIX_MSG + 'No processes to dump. Use --force to create empty dump file.');
      return cb ? cb(null, { success: true }) : that.exitCli(conf.SUCCESS_EXIT);
    }

    // Backup existing dump
    try {
      if (fs.existsSync(conf.DUMP_FILE_PATH)) {
        fs.copyFileSync(conf.DUMP_FILE_PATH, conf.DUMP_BACKUP_FILE_PATH);
      }
    } catch (e) {
    ...
    try {
      fs.writeFileSync(conf.DUMP_FILE_PATH, JSON.stringify(envList, null, 2));
      Common.printOut(conf.PREFIX_MSG + 'Successfully saved %d processes to %s', envList.length, conf.DUMP_FILE_PATH);
    } catch (e) {
      Common.printError(e);
      return cb ? cb(Common.retErr(e)) : that.exitCli(conf.ERROR_EXIT);
    }
 ...
 */
API.prototype.resurrect = function(cb) {
  var that = this;
  var conf = that._conf || require('../constants');

  var dumpFile = conf.DUMP_FILE_PATH;

  if (!fs.existsSync(dumpFile)) {
    Common.printError(conf.PREFIX_MSG_ERR + 'No dump file found at ' + dumpFile);
    return cb ? cb(new Error('No dump file')) : that.exitCli(conf.ERROR_EXIT);
  }

  var rawData;
  try {
    rawData = fs.readFileSync(dumpFile, 'utf8');
  } catch (e) {
    Common.printError(conf.PREFIX_MSG_ERR + 'Error reading dump file: ' + e.message);
    return cb ? cb(e) : that.exitCli(conf.ERROR_EXIT);
  }

  var envList;
  try {
    envList = JSON.parse(rawData);
  } catch (e) {
    Common.printError(conf.PREFIX_MSG_ERR + 'Corrupted dump file: ' + e.message);
    return cb ? cb(e) : that.exitCli(conf.ERROR_EXIT);
  }

  if (!Array.isArray(envList) || envList.length === 0) {
    Common.printOut(conf.PREFIX_MSG + 'No processes to resurrect.');
    return cb ? cb(null, []) : that.exitCli(conf.SUCCESS_EXIT);
  }

  Common.printOut(conf.PREFIX_MSG + 'Resurrecting %d processes...', envList.length);

  var async = require('async');
  async.eachSeries(envList, function(env, next) {
    // Reconstruct start options from saved pm2_env
    var opts = {
      name: env.name,
      namespace: env.namespace || 'default'
    };

    if (env.pm_cwd) opts.cwd = env.pm_cwd;
    if (env.exec_interpreter) opts.interpreter = env.exec_interpreter;
    if (env.node_args && env.node_args.length) opts.node_args = env.node_args;
    if (env.args && env.args.length) opts.args = env.args;
    if (env.instances && env.instances > 1) opts.instances = env.instances;
    if (env.env) opts.env = env.env;
    if (env.autorestart !== undefined) opts.autorestart = env.autorestart;
    if (env.max_memory_restart) opts.max_memory_restart = env.max_memory_restart;
    if (env.restart_delay) opts.restart_delay = env.restart_delay;
    if (env.max_restarts) opts.max_restarts = env.max_restarts;

    var script = env.pm_exec_path;
    if (!script) {
      Common.printError(conf.PREFIX_MSG_ERR + 'Skipping process ' + env.name + ': no script path');
      return next();
    }

    Common.printOut(conf.PREFIX_MSG + 'Resurrecting: %s', env.name);
    that.start(script, opts, function(err) {
      if (err) {
        Common.printError(conf.PREFIX_MSG_ERR + 'Failed to resurrect ' + env.name + ': ' + (err.message || err));
      }
      next(); // Continue even on error
    });
  }, function() {
    Common.printOut(conf.PREFIX_MSG + 'Resurrection complete.');
    return cb ? cb(null, envList) : that.speedList(0);
  });
};
 ...
 * Clear dump file
 */
API.prototype.clearDump = function(cb) {
  var that = this;
  var conf = that._conf || require('../constants');

  try {
    if (fs.existsSync(conf.DUMP_FILE_PATH)) {
      fs.unlinkSync(conf.DUMP_FILE_PATH);
    }
    if (fs.existsSync(conf.DUMP_BACKUP_FILE_PATH)) {
      fs.unlinkSync(conf.DUMP_BACKUP_FILE_PATH);
    }
    Common.printOut(conf.PREFIX_MSG + 'Dump file cleared.');
  } catch (e) {
    Common.printError(e);
    return cb ? cb(e) : that.exitCli(conf.ERROR_EXIT);
  }
