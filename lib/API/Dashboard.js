...
Dashboard.init = function() {
  ...
  this.box4 = blessed.text({
    content: ' left/right: switch boards | up/down/mouse: scroll | Ctrl-C: exit  ',
    left: '0%',
    top: '95%',
    width: '100%',
    height: '6%',
...
Dashboard.refresh = function(processes) {
  ...
  for (var i = 0; i < processes.length; i++) {
    ...
    // render the logBox
    let process_id = proc.pm2_env.pm_id
    ...
    this.metadataBox.setLine(9, 'Exec mode             ' + (proc.pm2_env.exec_mode == 'systemd' ? '{green-fg}{bold}systemd{/}' : proc.pm2_env.exec_mode == 'fork_mode' ? '{bold}fork{/}' : '{blue-fg}{bold}cluster{/}'));
