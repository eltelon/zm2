...
function deployHelper() {
  ...
  console.log('    $ zm2 deploy ecosystem.config.js production setup');
  console.log('');
  console.log('    Then deploy new code:');
  console.log('    $ zm2 deploy ecosystem.config.js production');
  console.log('');
  console.log('    If I want to revert to the previous commit:');
  console.log('    $ zm2 deploy ecosystem.config.js production revert 1');
  console.log('');
  console.log('    Execute a command on remote server:');
  console.log('    $ zm2 deploy ecosystem.config.js production exec "zm2 restart all"');
  console.log('');
  console.log('    ZM2 will look by default to the ecosystem.config.js file so you dont need to give the file name:');
  console.log('    $ zm2 deploy production');
  console.log('    Else you have to tell ZM2 the name of your ecosystem file');
  console.log('');
  console.log('    More examples in https://github.com/zappinginc/zm2');
  console.log('');
};
...
module.exports = function(CLI) {
  CLI.prototype.deploy = function(file, commands, cb) {
    ...
    if (!json_conf.deploy[env]['post-deploy']) {
      json_conf.deploy[env]['post-deploy'] = 'zm2 startOrRestart ' + file + ' --env ' + env;
    }
