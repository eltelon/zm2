...
module.exports = function(CLI) {
    ...
    that.Client.executeRemote('getReport', {}, function(err, report) {
      ...
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        Log.tail([{
          ...
          console.log(chalk.bold.green('Please copy/paste the above report in your issue on https://github.com/zappinginc/zm2/issues'));
  ...
  CLI.prototype.dashboard = function(cb) {
    ...
    // Track active log streams keyed by service name to avoid duplicates
    var logStreams = {};

    function startLogStream(proc) {
      var svcName = 'zm2-' + proc.pm2_env.name;
      if (logStreams[svcName]) return;

      var stream = Systemd.streamLogs(svcName, 20);
      logStreams[svcName] = stream;
      ...
      stream.stdout.on('data', function(data) {
        ...
        lines.forEach(function(line) {
          ...
          Dashboard.log('out', {
            process: { pm_id: proc.pm2_env.pm_id, name: proc.pm2_env.name },
            data: line
          });
        });
      });
      ...
      stream.on('exit', function() {
        delete logStreams[svcName];
      });
    }

    process.on('SIGINT', function() {
      Object.keys(logStreams).forEach(function(key) {
        try { logStreams[key].kill(); } catch(e) {}
      });
    ...
    function refreshDashboard() {
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        // Start a journalctl stream for each new process
        if (list) {
