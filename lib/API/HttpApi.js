...
function formatPrometheusMetrics(processes) {
  ...
  processes.forEach(function(proc) {
    var uptime = proc.pm2_env.pm_uptime ? Math.max(0, Math.floor((Date.now() - proc.pm2_env.pm_uptime) / 1000)) : 0;
...
function readBody(req, cb) {
  ...
  req.on('error', function() {
    if (!aborted) cb(new Error('Request aborted'));
  });
  req.on('end', function() {
    if (aborted) return;
...
function createServer(api, apiKey) {
  var server = http.createServer(function(req, res) {
    ...
    console.log('[ZM2 API] %s %s', method, pathname);

    var origEnd = res.end.bind(res);
    res.end = function() {
      console.log('[ZM2 API] %s %s -> %d (%dms)', method, pathname, res.statusCode, Date.now() - reqStart);
    ...
    switch (route.handler) {
      case 'metrics':
        ...
        api.Client.executeRemote('getMonitorData', {}, function(err, list) {
          console.log('[ZM2 API] getMonitorData took %dms, %d processes', Date.now() - t0, list ? list.length : 0);
          if (err) {
            console.error('[ZM2 API] getMonitorData error: %s', err.message);
      ...
      case 'listProcesses':
        var t1 = Date.now();
        api.Client.executeRemote('getMonitorData', {}, function(err, list) {
          console.log('[ZM2 API] getMonitorData took %dms, %d processes', Date.now() - t1, list ? list.length : 0);
        ...
        readBody(req, function(err, data) {
          ...
          if (data.env) {
            if (typeof data.env !== 'object' || Array.isArray(data.env)) {
              return sendJson(res, 400, { success: false, error: 'env must be a plain object' });
            }
            opts.env = data.env;
          }
      ...
      case 'startProcess':
        api.Client.executeRemote('startProcessId', { id: route.name }, function(err) {
          if (err) {
            var code = /not found/i.test(err.message) ? 404 : 500;
