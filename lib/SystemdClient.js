...
SystemdClient.prototype._getMonitorData = function(cb) {
  ...
  // Fetch all service statuses in a single systemctl call
  var t0 = Date.now();
  var bulkStatus = Systemd.getStatusBulk(serviceNames);
  console.log('[ZM2] getStatusBulk: %dms for %d services', Date.now() - t0, serviceNames.length);
  ...
  serviceNames.forEach(function(svcName, idx) {
    ...
    var status = bulkStatus[svcName] || defaultStatus;
  ...
  // Build pid->name map for logging
  var pidToName = {};
  Object.keys(pidToIndex).forEach(function(pid) {
    var idx = pidToIndex[parseInt(pid, 10)];
    pidToName[pid] = results[idx] && results[idx].pm2_env && results[idx].pm2_env.name;
  });

  var t1 = Date.now();
  pidusage(pids, function(err, stats) {
    var elapsed = Date.now() - t1;
    console.log('[ZM2] pidusage: %dms for %d pids', elapsed, pids.length);
    if (elapsed > 1000) {
      // Log each pid with its name to identify the slow one
      pids.forEach(function(pid) {
        var name = pidToName[pid] || '?';
        var got = stats && stats[pid] ? 'ok' : 'missing';
        console.log('[ZM2] pidusage slow - pid %d (%s): %s', pid, name, got);
      });
    }
    if (!err && stats) {
...
SystemdClient.prototype.executeRemote = function(method, app_conf, fn) {
  ...
  console.error('[zm2-debug] executeRemote method=%s', method);
  ...
  try {
    ...
    if (instances > 1) {
      ...
      for (var i = 0; i < instances; i++) {
        try { Systemd.enable(svcName + '@' + i); } catch (e) {}
      ...
      try { Systemd.enable(svcName); } catch (e) {}
  ...
  try {
    ...
    if (entry.instances > 1) {
      for (var i = 0; i < entry.instances; i++) {
        ...
        try { Systemd.disable(entry.serviceName + '@' + i); } catch (e) {}
      }
    } else {
      Systemd.stop(entry.serviceName);
      try { Systemd.disable(entry.serviceName); } catch (e) {}
    }
...
SystemdClient.prototype._startProcess = function(opts, cb) {
  try {
    ...
    if (entry.instances > 1) {
      for (var i = 0; i < entry.instances; i++) {
        try { Systemd.enable(entry.serviceName + '@' + i); } catch (e) {}
        Systemd.start(entry.serviceName + '@' + i);
      }
    } else {
      try { Systemd.enable(entry.serviceName); } catch (e) {}
...
SystemdClient.prototype._restartProcess = function(opts, cb) {
  try {
    ...
    // Only rewrite env file if env contains actual variables (not just current_conf wrapper)
    var envKeys = env ? Object.keys(env).filter(function(k) { return k !== 'current_conf'; }) : [];
    console.error('[zm2-debug] _restartProcess env keys (%d): %j', envKeys.length, envKeys.slice(0, 20));
