...
/**
 * Set exec_mode to systemd (fork/cluster modes removed).
 */
Common.sink.determineExecMode = function(app) {
  app.exec_mode = 'systemd';
...
Common.safeExtend = function(origin, add){
  ...
  var keysToIgnore = ['name', 'exec_mode', 'env', 'args', 'pm_cwd', 'exec_interpreter', 'pm_exec_path', 'node_args', 'pm_out_log_path', 'pm_err_log_path', 'pm_pid_path', 'pm_id', 'status', 'pm_uptime', 'created_at', 'windowsHide', 'username', 'kill_retry_time', 'prev_restart_delay', 'instance_var', 'unstable_restarts', 'restart_time', 'command', 'filter_env', 'versioning', 'instances', 'autostart', 'autorestart', 'stop_exit_codes', 'unstable_restart', 'exit_code'];
...
Common.verifyConfs = function(appConfs) {
  ...
  for (var i = 0; i < appConfs.length; i++) {
    ...
    if (typeof(app.merge_logs) == 'undefined') {
      app.merge_logs = true;
    }
